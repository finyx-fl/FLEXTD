//Copyright 2026 The flextd V3 Project Authors finyx-fl 
/**
 * flextd-physique v3.0 - ULTRA PHYSICS ENGINE merged into Flextd v3
 * Includes: SPH Fluids + Steam/Smoke + Cloth + Cars + RigidBody + Gas
 */
const REVISION = 'V3'
class Vec3 {
  ...
  multiplyScalar(s){return this.mul(s)}
  ...
  norm(){let l=this.len()||1; this.x/=l; this.y/=l; this.z/=l; return this}
  normalize(){return this.norm()}
  dist(v){return this.clone().sub(v).len()}
...
class RigidBody {
  constructor({mass=1,pos=new Vec3(),vel=new Vec3(),shape='box',size=new Vec3(1,1,1),friction=0.5,restitution=0.3,isStatic=false}={}){
    this.pos=pos.clone(); this.vel=vel.clone(); this.force=Vec3.zero()
    this.mass=isStatic?Infinity:mass; this.invMass=isStatic?0:1/mass
    this.shape=shape; this.size=size.clone()
    this.friction=friction; this.restitution=restitution
    this.isStatic=isStatic; this.angularVel=Vec3.zero(); this.torque=Vec3.zero()
    this.inertia = shape==='sphere' ? (2/5)*mass*size.x*size.x : mass
    this.invInertia = isStatic?0:1/this.inertia
  }
  applyForce(f, point=null){
    this.force.add(f)
    if(point){ let r=point.clone().sub(this.pos); this.torque.add(r.cross(f)) }
  }
  applyImpulse(impulse, point=null){
    this.vel.addScaled(impulse,this.invMass)
    if(point){ let r=point.clone().sub(this.pos); this.angularVel.addScaled(r.cross(impulse),this.invInertia) }
  }
  integrate(dt){
    if(this.isStatic) return
    let acc = this.force.clone().mul(this.invMass)
    this.vel.addScaled(acc,dt)
    this.vel.mul(0.999)
    this.pos.addScaled(this.vel,dt)
    this.angularVel.addScaled(this.torque,this.invInertia*dt)
    this.angularVel.mul(0.98)
    this.force=Vec3.zero(); this.torque=Vec3.zero()
  }
}

class SPHFluid {
  constructor({count=600, restDensity=1000, gasConstant=200, viscosity=3.5, surfaceTension=0.2, restLength=0.6}={}){
    this.particles=[]
    this.restDensity=restDensity; this.k=gasConstant; this.mu=viscosity; this.sigma=surfaceTension
    this.h=restLength; this.h2=restLength*restLength
    this.mass=1; this.damping=0.8
    for(let i=0;i<count;i++){
      this.particles.push({
        pos:new Vec3((Math.random()-0.5)*5, Math.random()*5+5, (Math.random()-0.5)*5),
        vel:Vec3.zero(), force:Vec3.zero(), density:0, pressure:0, neighbors:[]
      })
    }
  }
  Wpoly6(r,h){ if(r>=0&&r<=h){ let t=h*h-r*r; return 315/(64*Math.PI*Math.pow(h,9))*t*t*t } return 0 }
  Wpoly6Grad(rVec,h){ let r=rVec.len(); if(r>0&&r<=h){ let t=h*h-r*r; let f=-945/(32*Math.PI*Math.pow(h,9))*t*t; return rVec.clone().norm().mul(f) } return Vec3.zero() }
  WspikyGrad(rVec,h){ let r=rVec.len(); if(r>0&&r<=h){ let f=-45/(Math.PI*Math.pow(h,6))*Math.pow(h-r,2)/r; return rVec.clone().norm().mul(f) } return Vec3.zero() }
  WviscLap(r,h){ if(r>=0&&r<=h) return 45/(Math.PI*Math.pow(h,6))*(h-r); return 0 }
  update(dt, gravity){
    // FAST PATH: use SpatialHash if available
    if(window.SpatialHash3D && !this._hash) { this._hash = new SpatialHash3D(this.h); }
    if(this._hash){
      this._hash.clear();
      for(let p of this.particles) this._hash.insert(p);
      for(let p of this.particles){
        p.density=0; p.neighbors=[];
        const neigh = this._hash.queryRadius(p.pos.x, p.pos.y, p.pos.z, this.h);
        for(let q of neigh){
          let rVec=p.pos.clone().sub(q.pos); let r=rVec.len();
          if(r<this.h){ p.neighbors.push({p:q,rVec,r}); p.density+=this.mass*this.Wpoly6(r,this.h); }
        }
      }
    } else {
      for(let p of this.particles){
      ...
      p.density=Math.max(p.density,this.restDensity*0.3)
      p.pressure=this.k*(p.density-this.restDensity)
    }
    for(let p of this.particles){
      let fPressure=Vec3.zero(), fVisc=Vec3.zero()
      for(let n of p.neighbors){
        if(n.p===p) continue
        let grad = this.WspikyGrad(n.rVec,this.h)
        let avgP=(p.pressure+n.p.pressure)/(2*n.p.density)
        fPressure.addScaled(grad, -this.mass*avgP)
        let velDiff=n.p.vel.clone().sub(p.vel)
        let lap=this.WviscLap(n.r,this.h)
        fVisc.addScaled(velDiff, this.mu*this.mass/n.p.density*lap)
      }
      p.force = fPressure.clone().add(fVisc).add(new Vec3(0,gravity.y*p.density,0))
    }
    for(let p of this.particles){
      let acc=p.force.clone().mul(1/p.density)
      p.vel.addScaled(acc,dt); p.vel.mul(this.damping)
      p.pos.addScaled(p.vel,dt)
      if(p.pos.y<0){ p.pos.y=0; p.vel.y*=-0.3; p.vel.x*=0.8; p.vel.z*=0.8 }
    }
  }
}

class SteamSmokeSystem {
  ...
  spawn(randomY=false){
    return {
      pos:new Vec3((Math.random()-0.5)*0.5, randomY?Math.random()*2:0, (Math.random()-0.5)*0.5),
      vel:new Vec3((Math.random()-0.5)*0.3, Math.random()*0.5+0.8, (Math.random()-0.5)*0.3),
      life: Math.random(), temp: 80+Math.random()*60, size: 0.1+Math.random()*0.4,
      vorticity: new Vec3((Math.random()-0.5)*2,0,(Math.random()-0.5)*2)
    }
  }
  update(dt, origin=Vec3.zero()){
    for(let i=0;i<this.particles.length;i++){
      let p=this.particles[i]
      let buoyancy = (p.temp/100)*1.5
      p.vel.y += buoyancy*dt - 0.15*dt
      p.vel.addScaled(p.vorticity, dt*0.3)
      p.vorticity.x += (Math.random()-0.5)*dt
      p.vorticity.z += (Math.random()-0.5)*dt
      p.vel.x += Math.sin(p.life*6+p.pos.y)*0.02
      p.vel.z += Math.cos(p.life*5+p.pos.y)*0.02
      p.temp -= dt*12
      p.size += dt*0.35
      p.pos.addScaled(p.vel,dt)
      p.life -= dt*0.25
      if(p.life<=0 || p.temp<20){ this.particles[i]=this.spawn(false); this.particles[i].pos.add(origin) }
    }
  }
}

class Cloth {
  constructor(w=10,h=10,segments=12){
    this.points=[]; this.sticks=[]; this.pinned=new Set()
    let dx=w/segments, dy=h/segments
    for(let y=0;y<=segments;y++) for(let x=0;x<=segments;x++){
      this.points.push({pos:new Vec3(x*dx-w/2,5+y*dy,0), old:new Vec3(x*dx-w/2,5+y*dy,0), locked:false})
    }
    let idx=(x,y)=>y*(segments+1)+x
    for(let y=0;y<=segments;y++) for(let x=0;x<=segments;x++){
      if(x<segments) this.sticks.push({a:idx(x,y),b:idx(x+1,y),len:dx})
      if(y<segments) this.sticks.push({a:idx(x,y),b:idx(x,y+1),len:dy})
      if(x<segments&&y<segments){ this.sticks.push({a:idx(x,y),b:idx(x+1,y+1),len:Math.sqrt(dx*dx+dy*dy)}); this.sticks.push({a:idx(x+1,y),b:idx(x,y+1),len:Math.sqrt(dx*dx+dy*dy)}) }
    }
    for(let x=0;x<=segments;x++) this.points[idx(x,segments)].locked=true
  }
  update(dt, gravity, iterations=12){
    for(let p of this.points){
      if(p.locked) continue
      let vel=p.pos.clone().sub(p.old); vel.mul(0.99)
      let next=p.pos.clone().add(vel).addScaled(gravity,dt*dt)
      p.old=p.pos.clone(); p.pos=next
    }
    for(let k=0;k<iterations;k++) for(let s of this.sticks){
      let a=this.points[s.a], b=this.points[s.b]
      let delta=b.pos.clone().sub(a.pos); let d=delta.len()||0.001; let diff=(d-s.len)/d
      if(!a.locked) a.pos.addScaled(delta,0.5*diff)
      if(!b.locked) b.pos.addScaled(delta,-0.5*diff)
    }
  }
}
...
class Wheel {
  constructor(offset, radius=0.4){
    this.offset=offset; this.radius=radius
    this.suspensionRest=0.5; this.suspensionStiffness=2800; this.damping=320
    this.compression=0; this.force=0; this.grounded=false
    this.spin=0; this.steerAngle=0
  }
}

class Car {
  constructor(pos=new Vec3(0,1,0)){
    this.body=new RigidBody({mass:1800,pos, size:new Vec3(1.8,1,4.2), friction:0.9})
    this.wheels=[
      new Wheel(new Vec3(-0.9,-0.5,1.3)), new Wheel(new Vec3(0.9,-0.5,1.3)),
      new Wheel(new Vec3(-0.9,-0.5,-1.3)), new Wheel(new Vec3(0.9,-0.5,-1.3))
    ]
    this.engineForce=0; this.brakeForce=0; this.steerInput=0
    this.maxSteer=Math.PI/5; this.maxEngine=9000; this.maxBrake=5000
    this.driftFactor=0.92
  }
  setInput(throttle, brake, steer){
    this.engineForce=throttle*this.maxEngine
    this.brakeForce=brake*this.maxBrake
    this.steerInput=steer
  }
  update(dt, gravity){
    for(let i=0;i<4;i++){
      let w=this.wheels[i]
      let worldPos=this.body.pos.clone().add(w.offset)
      let rayLen=w.suspensionRest+w.radius
      let dist=worldPos.y
      if(dist<rayLen){
        w.grounded=true; w.compression=rayLen-dist
        let springForce=w.suspensionStiffness*w.compression
        let dampForce=w.damping*(-this.body.vel.y)
        w.force=springForce+dampForce
        this.body.applyForce(new Vec3(0,w.force,0), worldPos)
        let isFront=i<2
        w.steerAngle = isFront? this.steerInput*this.maxSteer : 0
        if(isFront){
          let lateral = -this.body.vel.x*1200*w.compression
          this.body.applyForce(new Vec3(lateral,0,0), worldPos)
        } else {
          this.body.applyForce(new Vec3(0,0,this.engineForce*dt*w.compression), worldPos)
          this.body.vel.x*=this.driftFactor
        }
        if(this.brakeForce>0){
          let brakeDir=this.body.vel.clone().norm().mul(-1)
          this.body.applyForce(brakeDir.clone().mul(this.brakeForce*dt*w.compression))
        }
      } else { w.grounded=false; w.force=0 }
    }
    this.body.applyForce(gravity.clone().mul(this.body.mass*0.3))
    this.body.integrate(dt)
    if(this.body.pos.y<1) this.body.pos.y=1
  }
}

class FlextdWorld {
  constructor({gravity=new Vec3(0,-9.81,0)}={}){
    this.gravity=gravity; this.bodies=[]; this.fluids=[]; this.smokes=[]; this.cloths=[]; this.cars=[]
    this.time=0
  }
  addBody(b){this.bodies.push(b); return b}
  addFluid(f){this.fluids.push(f); return f}
  addSmoke(s){this.smokes.push(s); return s}
  addCloth(c){this.cloths.push(c); return c}
  addCar(c){this.cars.push(c); return c}
  handleCollisions(){
    for(let i=0;i<this.bodies.length;i++) for(let j=i+1;j<this.bodies.length;j++){
      let a=this.bodies[i], b=this.bodies[j]; if(a.isStatic&&b.isStatic) continue
      let d=a.pos.clone().sub(b.pos).len()
      let minDist=(a.size.x+b.size.x)*0.5
      if(d<minDist&&d>0.001){
        let n=a.pos.clone().sub(b.pos).norm()
        let overlap=minDist-d
        let totalInv=a.invMass+b.invMass||1
        if(!a.isStatic) a.pos.addScaled(n, overlap*(a.invMass/totalInv))
        if(!b.isStatic) b.pos.addScaled(n, -overlap*(b.invMass/totalInv))
        let relVel=a.vel.clone().sub(b.vel).dot(n)
        if(relVel<0){
          let e=Math.min(a.restitution,b.restitution)
          let j=-(1+e)*relVel/totalInv
          let imp=n.clone().mul(j)
          if(!a.isStatic) a.vel.addScaled(imp,a.invMass)
          if(!b.isStatic) b.vel.addScaled(imp,-b.invMass)
        }
      }
    }
    for(let b of this.bodies) if(!b.isStatic&&b.pos.y<b.size.y*0.5){ b.pos.y=b.size.y*0.5; b.vel.y*=-b.restitution; b.vel.x*=1-b.friction*0.2; b.vel.z*=1-b.friction*0.2 }
  }
  step(dt=1/60){
    this.time+=dt
    for(let b of this.bodies) if(!b.isStatic){ b.applyForce(this.gravity.clone().mul(b.mass)); b.integrate(dt) }
    for(let f of this.fluids) f.update(dt,this.gravity)
    for(let s of this.smokes) s.update(dt)
    for(let c of this.cloths) c.update(dt,this.gravity)
    for(let car of this.cars) car.update(dt,this.gravity)
    this.handleCollisions()
  }
}

// Legacy simple physics kept for backward compat with FlextdEngine v3
class FlextdPhysicsLegacy{
...
this.physics=new FlextdPhysicsLegacy(); this.audio=new FlextdAudio();
...
console.log('%c[Flextd v3 ONE FILE - 1MB MAX POWER] Ready', 'color:#ff00ff; background:#000; padding:4px; font-weight:bold; font-size:14px');
}
...
// ===== Flextd v3 Integration Helpers =====
FlextdEngine.prototype.enableUltraPhysics = function(opts={}){
  this.ultraWorld = new FlextdWorld({gravity:new Vec3(0,-9.81,0)});
  this.ultraFluid = null;
  this.ultraSmoke = null;
  this.ultraCloth = null;
  this.ultraCar = null;
  console.log('%c[Flextd v3 ULTRA PHYSICS] FlextdWorld + SPH + Cloth + Car Ready','color:#00ffff;background:#000;padding:4px');
  return this.ultraWorld;
};
FlextdEngine.prototype.addFluid = function(count=400){
  if(!this.ultraWorld) this.enableUltraPhysics();
  const f=new SPHFluid({count});
  this.ultraWorld.addFluid(f);
  this.ultraFluid=f;
  return f;
};
FlextdEngine.prototype.addSmoke = function(count=300){
  if(!this.ultraWorld) this.enableUltraPhysics();
  const s=new SteamSmokeSystem(count);
  this.ultraWorld.addSmoke(s);
  this.ultraSmoke=s;
  return s;
};
FlextdEngine.prototype.addClothMesh = function(w=8,h=5,seg=10){
  if(!this.ultraWorld) this.enableUltraPhysics();
  const c=new Cloth(w,h,seg);
  this.ultraWorld.addCloth(c);
  this.ultraCloth=c;
  return c;
};
FlextdEngine.prototype.addCarPhysics = function(x=0,y=2,z=0){
  if(!this.ultraWorld) this.enableUltraPhysics();
  const car=new Car(new Vec3(x,y,z));
  this.ultraWorld.addCar(car);
  this.ultraCar=car;
  return car;
};
...
window.FlextdWorld=FlextdWorld;
window.RigidBody=RigidBody;
window.SPHFluid=SPHFluid;
window.SteamSmokeSystem=SteamSmokeSystem;
window.Cloth=Cloth;
window.Car=Car;
window.FlextdPhysicsLegacy=FlextdPhysicsLegacy;
window.FlextdPhysics=FlextdPhysicsLegacy; // backward compat
...
console.log('%c[Flextd v3 ONE FILE] 🔥 PBR + Physics + FTD + GLB->FTD + ULTRA-GI (Vulkan Killer) - All in One File!', 'color:#ff00ff; background:#000; padding:6px; font-weight:bold; font-size:13px');

// ===== FlextdVoxel v3 - ADDED ULTRA LIGHT VOXEL CORE (same file, same letters) =====
/**
 * FlextdVoxel.js v3 - ULTIMATE SINGLE FILE VOXEL ENGINE
...
console.log('%c[Flextd v3 VOXEL EDITION] ✅ Same letters + VOXEL GOD - Greedy + Infinite', 'color:#00ff88;background:#000;padding:6px;font-weight:bold;font-size:13px');


// ====== SPATIAL HASH - CONTRIBUTION BY USER ======

/**
 * SpatialHash3D.js - نسخة Flextd المحسنة
 * من طرفك - مدموجة مع Vec3
 */
...
class SpatialHash3D {
  ...
  _getAABB(a) {
    if (a.pos) { // Vec3 particle
      return { minX: a.pos.x - this.cellSize*0.5, minY: a.pos.y - this.cellSize*0.5, minZ: a.pos.z - this.cellSize*0.5, maxX: a.pos.x + this.cellSize*0.5, maxY: a.pos.y + this.cellSize*0.5, maxZ: a.pos.z + this.cellSize*0.5 };
    }
    if ('x' in a) {
      return { minX: a.x, minY: a.y, minZ: a.z, maxX: a.x + (a.w ?? 0), maxY: a.y + (a.h ?? 0), maxZ: a.z + (a.d ?? 0) };
    }
    return a;
  }
  _getCellKeysForAABB(aabb) {
    const { minX, minY, minZ, maxX, maxY, maxZ } = this._getAABB(aabb);
    const keys = new Set();
  ...
  insert(obj) {
    if (this.objectCells.has(obj)) this.remove(obj);
    const keys = this._getCellKeysForAABB(obj);
  ...
  remove(obj) {
    ...
    for (const key of keys) {
      ...
      if (cell) { cell.delete(obj); if (cell.size === 0) this.cells.delete(key); }
    }
    this.objectCells.delete(obj);
    return this;
  }
  clear() { this.cells.clear(); this.objectCells.clear(); }
  query(aabb) {
    const keys = this._getCellKeysForAABB(aabb);
    const result = new Set();
    for (const key of keys) {
      const cell = this.cells.get(key);
      if (!cell) continue;
      for (const o of cell) result.add(o);
    }
    return [...result];
  }
  queryNeighbors(obj) { return this.query(obj).filter(o => o !== obj); }
  queryRadius(x, y, z, radius) {
    const aabb = { x: x - radius, y: y - radius, z: z - radius, w: radius * 2, h: radius * 2, d: radius * 2 };
    return this.query(aabb).filter(o=>{
      const ox = o.pos ? o.pos.x : o.x + (o.w?o.w/2:0);
      const oy = o.pos ? o.pos.y : o.y + (o.h?o.h/2:0);
      const oz = o.pos ? o.pos.z : o.z + (o.d?o.d/2:0);
  ...
  getPotentialPairs() {
    ...
    for (const cell of this.cells.values()) {
      ...
      for (let i=0;i<objs.length;i++) for(let j=i+1;j<objs.length;j++){
        ...
        const idA = a.id ?? a._hashId ?? objs[i], idB = b.id ?? b._hashId ?? objs[j];
        const id = String(idA)+"|"+String(idB);
        if(seen.has(id)) continue; seen.add(id); pairs.push([a,b]);
      }
    }
    return pairs;
  }
}
window.SpatialHash3D = SpatialHash3D;


// ====== COLLISION ======
/**
 * Flextd - Collision.js
 * SAT + AABB + Sphere Resolver - يخلي الاجسام ما يدوزوش من بعضياتهم
 */

class Collision {
  static aabbOverlap(a, b) {
    return (
      Math.abs(a.pos.x - b.pos.x) <= (a.size.x + b.size.x) * 0.5 &&
      Math.abs(a.pos.y - b.pos.y) <= (a.size.y + b.size.y) * 0.5 &&
      Math.abs(a.pos.z - b.pos.z) <= (a.size.z + b.size.z) * 0.5
    );
  }

  static sphereOverlap(a, b) {
    const r = a.size.x + b.size.x;
  ...
  static resolve(a, b) {
    ...
    const sx = (a.size.x + b.size.x) * 0.5;
    const sy = (a.size.y + b.size.y) * 0.5;
    const sz = (a.size.z + b.size.z) * 0.5;
    ...
    // فصل الاجسام
    const moveA = a.invMass / totalInvMass;
    const moveB = b.invMass / totalInvMass;
    
    a.pos.addScaled(normal, -depth * moveA);
    b.pos.addScaled(normal, depth * moveB);
    ...
    if (!a.isStatic) a.vel.sub(impulse.clone().mul(a.invMass));
    if (!b.isStatic) b.vel.add(impulse.clone().mul(b.invMass));
    
    // احتكاك
    const friction = Math.sqrt(a.friction * b.friction);
    const tangent = rv.clone().sub(normal.clone().mul(rv.dot(normal)));
    if (tangent.len() > 0.001) {
      tangent.norm();
      let jt = -rv.dot(tangent) / totalInvMass;
      jt = Math.max(-j * friction, Math.min(jt, j * friction));
      const frictionImpulse = tangent.clone().mul(jt);
      if (!a.isStatic) a.vel.sub(frictionImpulse.clone().mul(a.invMass));
      if (!b.isStatic) b.vel.add(frictionImpulse.clone().mul(b.invMass));
    }
  }
}

window.FlextdCollision = Collision;


// ====== PBR ======
/**
 * Flextd - RendererPBR.js
 * PBR + Shadows + Fog - يخلي الحديد يبان حديد
 */

class PBRMaterial {
  constructor({albedo=[0.8,0.8,0.8], metallic=0.0, roughness=0.5, emissive=0.0}={}){
    this.albedo = albedo;
...
const PBR_SHADERS = {
  vert: `#version 300 es
    ...
    out vec3 vPos, vNorm, vColor; out float vAO, vEmiss;
    void main(){
      vPos = aPos;
      vNorm = normalize(aNorm);
      vColor = aColor;
      vAO = aAO; vEmiss = aEmiss;
      gl_Position = uProj * uView * vec4(aPos, 1.0);
    }
  `,
  frag: `#version 300 es
    precision highp float;
    in vec3 vPos, vNorm, vColor; in float vAO, vEmiss;
    uniform vec3 uSunDir, uFogColor, uCamPos;
    uniform float uTime;
    out vec4 outColor;
    ...
    void main(){
      ...
      // Base lighting
      float diff = NoL * 0.8 + 0.2;
      
      // Simple PBR
      float roughness = 0.6;
      float spec = D_GGX(NoH, roughness) * 0.3;
      
      // AO + Fog
      float fogDist = length(uCamPos - vPos);
      float fog = 1.0 - exp(-fogDist*0.015);
      
      vec3 color = vColor * diff * vAO;
      color += vec3(spec);
      color += vec3(vEmiss * 1.5);
...
window.PBRMaterial = PBRMaterial;
window.PBR_SHADERS = PBR_SHADERS;


// ====== LOADER ======
/**
 * Flextd - Loader.js
 * GLTF + Texture Loader خفيف
 */
...
class FlextdLoader {
  ...
  // Simple OBJ parser (اخف من GLTF ويخدم فكلشي)
  static parseOBJ(text){
    ...
    for(let line of lines){
      ...
      if(parts[0]==='v'){
        ...
        // f v3/vt/vn
        for(let i=1;i<=3;i++){
          const idx = parts[i].split('/')[0];
          const pos = positions[parseInt(idx)-1];
  ...
  // Load heightmap to voxels
  static heightmapToVoxels(imgData, scale=10){
    const voxels=[];
    for(let y=0;y<imgData.height;y++){
      for(let x=0;x<imgData.width;x++){
        const i = (y*imgData.width + x)*4;
        const h = imgData.data[i] / 255 * scale;
        for(let z=0;z<h;z++) voxels.push({x, y:Math.floor(z), z:y, id:1});
      }
    }
    return voxels;
  }
...
window.FlextdLoader = FlextdLoader;


// ====== LOD ======
/**
 * Flextd - WorldLOD.js
 * Level of Detail لل Voxel - باش العالم ما يموتش
 */

class WorldLOD {
  constructor(){
    this.levels = [
      {dist: 0, chunkRes: 1, greedy: true},    // قريب: Full detail
      {dist: 80, chunkRes: 2, greedy: true},   // متوسط: نصف التفاصيل
      {dist: 160, chunkRes: 4, greedy: false}, // بعيد: ربع التفاصيل
    ];
  }
  
  getLevel(distance){
    for(let i=this.levels.length-1;i>=0;i--){
      if(distance >= this.levels[i].dist) return this.levels[i];
    }
    return this.levels[0];
  }
  
  shouldRender(chunkDist, renderDistance){
    return chunkDist <= renderDistance;
  }
  
  // Frustum culling بسيط
  isInFrustum(chunkPos, camPos, camDir, fov){
    const toChunk = {x: chunkPos.x - camPos.x, z: chunkPos.z - camPos.z};
    const dist = Math.hypot(toChunk.x, toChunk.z);
    if(dist > 200) {
      const dot = (toChunk.x*camDir.x + toChunk.z*camDir.z) / dist;
      if(dot < Math.cos(fov*0.7)) return false;
    }
    return true;
  }
  
  // Merge distant chunks
  mergeChunks(chunks){
    // ياخذ 4 chunks صغار ويرجع chunk كبير low-res
    if(chunks.length !== 4) return null;
    // تبسيط: يرجع متوسط الارتفاعات
    return {merged: true, source: chunks};
  }
}

window.WorldLOD = WorldLOD;
